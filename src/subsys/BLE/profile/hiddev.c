...
static gapRolesCBs_t hidDev_PeripheralCBs =
{
  ...
  hidDevRssiCB,                // When a valid RSSI is read from controller
  hidDevParamUpdateCB
};
...
uint16 HidDev_ProcessEvent( uint8 task_id, uint16 events )
{
  ...
  {
    GAPRole_ReadRssiCmd(gapConnHandle);
...
//  tmos_memcpy( noti.pValue, &rssi, noti.len );
//  if( rssi_Notify( gapConnHandle, &noti ) != SUCCESS )
//  {
...
static void hidDevDisconnected( void )
{
  ...
  tmos_stop_task( hidDevTaskId, SBP_READ_RSSI_EVT);
...
static void hidDevGapStateCB( gapRole_States_t newState, gapRoleEvent_t * pEvent )
{
  ...
  if ( newState == GAPROLE_CONNECTED )
  {
    ...
    LOG_INFO("gapConnHandle: %#x", gapConnHandle);
...
static void hidDevPairStateCB( uint16 connHandle, uint8 state, uint8 status )
{
    ...
    else if( state == GAPBOND_PAIRING_STATE_BOND_SAVED )
  {
    ...
    uint8_t addr_type;
    /**
     * NOTE: 绑定个数(SNVNum)配置为1， 
     * 且开启GAPBOND_ERASE_AUTO，则SNV中仅存在当前绑定地址。
     */
    ...
    tmos_memcpy(device_bond.ID[ID_Num].remote_addr, bond_info.publicAddr, 6);
    ...
    device_bond.ID[ID_Num].remote_addr_type = addr_type;  

    device_bond.ID[ID_Num].isbond = true;
